...
class OpenlayersPlugin:
    ...
    def initGui(self):
        ...
        #self.dlg.toolButton_map
        self.dlg.toolButton_map.setMenu(self._olMenu)
    ...
    def addLayer(self, layerType):
        ...
        if layer.isValid():
            ...
            #Setting map at back
            print("Set background map back")
            bridge = self.iface.layerTreeCanvasBridge()
            order = bridge.customLayerOrder()
            order.insert(-1, order.pop(order.index(layer.id()))) # vlayer to the bottom
